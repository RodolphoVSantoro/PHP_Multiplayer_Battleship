...
if(isset($_POST['coorx']))
{
		...
		if(isset($_SESSION['cx1']) && isset($_SESSION['cx2']))
		{
			if($_SESSION['cx1'] == $_SESSION['cx2'] && $_SESSION['cy1'] != $_SESSION['cy2'])
			{
				...
				if($subtracao == $tipodebarco)
				{
					...
					{
						if($_SESSION['CampoJogador2'][$cx][$x] !=0)
						{
							$sobrepondo = "v";
						}
					}
					if($sobrepondo == "f")
					{
						for($x=$menor;$x<=$maior;$x++)
						{
							$_SESSION['CampoJogador2'][$cx][$x] = 1;
						}
						$_SESSION['BarcosJogador2'] = $_SESSION['BarcosJogador2'] + 1;
						unset($_SESSION['cx1']);
						unset($_SESSION['cy1']);
						unset($_SESSION['cx2']);
						unset($_SESSION['cy2']);
			...
			else
			{
				if($_SESSION['cx1'] != $_SESSION['cx2'] && $_SESSION['cy1'] == $_SESSION['cy2'])
				{
					...
					if($subtracao == $tipodebarco)
					{
						...
						for($x=$menor;$x<=$maior;$x++)
						{
							if($_SESSION['CampoJogador2'][$x][$cy] !=0)
							{
								$sobrepondo = "v";
							}
						}
	...
	{
			$_SESSION["BarcosJogador2"] += 1;
			$a = $_POST['coorx'];
			$b = $_POST['coory'];
			$_SESSION["CampoJogador2"][$a][$b] = 1;
	}
}

if($_SESSION["BarcosJogador2"] == $_SESSION['NBarcos'])
{
$_SESSION["BarcosJogador2"]-=1;
unset($_SESSION['cx1']);
unset($_SESSION['cy1']);
...
$tipodebarco = $_SESSION['OrdemBarcos'][$n];
echo'<p id="jog">Selecione '.$_SESSION['NBarcos'].' Barcos<br/>'.$_SESSION['NomeJ2'];
if($tipodebarco == 0)
{
	echo "(Submarino)";
}
...
echo"<table id='tabjogador2'>";
for($i=0;$i<$_SESSION['TamanhoCampo'];$i++)
{
  echo "<tr>";
  for($j=0;$j<$_SESSION['TamanhoCampo'];$j++)
  {
    echo"<td><img src='agua2.jpg'></td>";
  }
  echo"</tr>";
}

echo "<table  id='tabjogador3'>";
for($i=0;$i<$_SESSION['TamanhoCampo'];$i++)
{
  echo "<tr>";
  for($j=0;$j<$_SESSION['TamanhoCampo'];$j++)
  {
    if($_SESSION["CampoJogador2"][$i][$j] == 0){
    echo"<td> 
             <form action='' method='post'>
               <input type='hidden' name='coorx' value=".$i.">
               <input type='hidden' name='coory' value=".$j.">
               <input type='image' src='desconhecido2.jpg'>
             </form>
	</td>";
    }
    else
